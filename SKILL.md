...
当前版本为1.5.0。它把教师阅读、后续文本分析和内部校验分开处理，最终交付HTML、DOCX和逐篇TXT。
...
脚本默认调用 `https://vocabprofiler.netlify.app/api/analyze`，逐篇分析完整正文。所有材料通过API合同校验后，内部包升级为schema 1.2；网络或响应校验失败时，停止本轮交付并报告原因。
...
- 每篇 `curriculum_vocabulary_coverage.status` 都是 `analyzed`。
- HTML可以离线打开；复制按钮会复制标题和全文；页面与打印预览都能显示全部材料。
- DOCX包含来源、教学适配、词汇覆盖、风险和教师决定，英文全文留在HTML与TXT中。
...
- 词汇覆盖以VocabProfiler返回的结果为准。API失败时保留现有工作文件并报告错误。
